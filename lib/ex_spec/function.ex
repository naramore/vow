defmodule ExSpec.Function do
  ...
  @doc false
  @spec new([ExSpec.t()] | nil, ExSpec.t() | nil, ExSpec.t() | nil) :: t
    ...
    with {:aspec, aspec} when not is_nil(aspec) <- {:aspec, spec.args},
      ...
      {:aspec, nil} ->
        with {:rspec, rspec} when not is_nil(rspec) <- {:rspec, spec.ret},
             {:ret, ret} <- {:ret, execute!(fun, args)},
             {:cret, {:ok, cret}} <- {:cret, ExSpec.conform(rspec, ret)} do
