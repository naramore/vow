...
defmodule StreamDataUtils do
  ...
  @doc """
  """
  @spec function(arity, t(a)) :: t((... -> a)) | no_return when a: term
  ...
  defdelegate pos_integer(),
    to: StreamData, as: :positive_integer
  ...
  @doc """
  """
  @spec keyword_of(t(a), [list_opt]) :: t(keyword(a)) when a: term
  ...
  defp split_date_and_time_opts(opts) do
    with {:min, nil} <- {:min, Keyword.get(opts, :min_datetime)},
      ...
      {:max, max} ->
        {[max_date: DateTime.to_date(max)],
         [max_time: DateTime.to_time(max)]}
      {:min, min} ->
        case Keyword.get(opts, :max_datetime) do
          nil ->
            {[min_date: DateTime.to_date(min)],
             [min_time: DateTime.to_time(min)]}
          max ->
            {[min_date: DateTime.to_date(min),
              max_date: DateTime.to_date(max)],
             [min_time: DateTime.to_time(min),
  ...
  @doc false
  @spec get_date_range(keyword) :: {Date.t, nil} | {nil, Date.t} | {nil, nil} | Date.Range.t
