defmodule ExSpecData do
  ...
  @spec non_recur_spec(keyword) :: stream_data(ExSpec.t())
  ...
  @spec recur_spec(keyword) :: stream_data(ExSpec.t())
  ...
  @spec func(keyword) :: stream_data(ExSpec.Func.t())
  def func(opts \\ []) do
    pred_fun(opts)
    |> StreamData.map(&ExSpec.Func.new(&1, ~s<¯\_(ツ)_/¯>))
  end
  ...
  @spec list_of(stream_data | nil, keyword) :: stream_data(ExSpec.List.t())
  def list_of(child_data \\ nil, opts \\ []) do
    child_data
    |> process(opts)
    |> StreamData.map(&ExSpec.list_of(&1, opts))
  end

  @spec maybe(stream_data | nil, keyword) :: stream_data(ExSpec.Maybe.t())
  def maybe(child_data \\ nil, opts \\ []) do
    child_data
    |> process(opts)
    |> StreamData.map(&ExSpec.maybe/1)
  end

  @spec nilable(stream_data | nil, keyword) :: stream_data(ExSpec.Nilable.t())
  def nilable(child_data \\ nil, opts \\ []) do
    child_data
    |> process(opts)
    |> StreamData.map(&ExSpec.nilable/1)
  end

  @spec oom(stream_data | nil, keyword) :: stream_data(ExSpec.OneOrMore.t())
  def oom(child_data \\ nil, opts \\ []) do
    child_data
    |> process(opts)
    |> StreamData.map(&ExSpec.oom/1)
  end

  @spec zom(stream_data | nil, keyword) :: stream_data(ExSpec.ZeroOrMore.t())
  def zom(child_data \\ nil, opts \\ []) do
    child_data
    |> process(opts)
    |> StreamData.map(&ExSpec.zom/1)
  end

  @spec map_of(stream_data | nil, keyword) :: stream_data(ExSpec.Map.t())
  def map_of(child_data \\ nil, opts \\ []) do
    data = process(child_data, opts)

    StreamData.tuple({data, data})
    |> StreamData.map(fn {k, v} ->
      ExSpec.map_of(k, v, opts)
    end)
  end

  @spec also(stream_data | nil, keyword) :: stream_data(ExSpec.Also.t())
  def also(child_data \\ nil, opts \\ []) do
    child_data
    |> process(opts)
    |> StreamData.list_of(opts)
    |> StreamData.map(&ExSpec.also/1)
  end

  @spec amp(stream_data | nil, keyword) :: stream_data(ExSpec.Amp.t())
  def amp(child_data \\ nil, opts \\ []) do
    child_data
    |> process(opts)
    |> StreamData.list_of(opts)
    |> StreamData.map(&ExSpec.amp/1)
  end
  ...
  @spec merge(stream_data | nil, keyword) :: stream_data(ExSpec.Merge.t())
  def merge(child_data \\ nil, opts \\ []) do
    ...
    |> process(opts)
    |> StreamData.list_of(opts)
    |> StreamData.map(&ExSpec.merge(&1, merge_fun))
  end

  @spec cat(stream_data | nil, keyword) :: stream_data(ExSpec.Cat.t())
  def cat(child_data \\ nil, opts \\ []) do
    child_data
    |> process(opts)
    |> named_specs(opts)
    |> StreamData.map(&ExSpec.cat/1)
  end

  @spec alt(stream_data | nil, keyword) :: stream_data(ExSpec.Alt.t())
  def alt(child_data \\ nil, opts \\ []) do
    child_data
    |> process(opts)
    |> named_specs(opts)
    |> StreamData.map(&ExSpec.alt/1)
  end

  @spec one_of(stream_data | nil, keyword) :: stream_data(ExSpec.OneOf.t())
  def one_of(child_data \\ nil, opts \\ []) do
    child_data
    |> process(opts)
    |> named_specs(opts)
    |> StreamData.map(&ExSpec.one_of/1)
  end
